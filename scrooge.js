...
const DEFAULT_COVER_URL = "https://cdn.jsdelivr.net/gh/daknux/covers@main";
const DEFAULT_HTML_URL = "https://cdn.jsdelivr.net/gh/daknux/html@main";

let zonesurls = [...DEFAULT_ZONE_URLS];
let zonesURL = zonesurls[Math.floor(Math.random() * zonesurls.length)];
let coverURL = DEFAULT_COVER_URL;
let htmlURL = DEFAULT_HTML_URL;
let zones = [];
let popularityData = {};
...
// Pulls { owner, repo, version } out of a jsDelivr gh URL like
// https://cdn.jsdelivr.net/gh/owner/repo@version/path.json
...
function loadSourceSettings() {
    try {
        const saved = JSON.parse(localStorage.getItem('daknuxSourceSettings'));
        if (!saved) return;
        if (Array.isArray(saved.zonesURLs) && saved.zonesURLs.length) {
            zonesurls = saved.zonesURLs.map(u => String(u).trim()).filter(Boolean);
            if (zonesurls.length) zonesURL = zonesurls[0];
        }
        if (typeof saved.coverURL === 'string' && saved.coverURL.trim()) coverURL = saved.coverURL.trim();
        if (typeof saved.htmlURL === 'string' && saved.htmlURL.trim()) htmlURL = saved.htmlURL.trim();
    } catch (error) {
...
function saveSourceSettings() {
    const zonesVal = document.getElementById('settingZonesURL').value;
    const coversVal = document.getElementById('settingCoversURL').value.trim();
    const htmlVal = document.getElementById('settingHTMLURL').value.trim();
    zonesurls = zonesVal.split(/\n/).map(u => u.trim()).filter(Boolean);
    if (!zonesurls.length || !coversVal || !htmlVal) {
        alert("All source fields must be filled in.");
        return;
    }
    zonesURL = zonesurls[0];
    coverURL = coversVal;
    htmlURL = htmlVal;
...
function resetSourceSettings() {
    zonesurls = [...DEFAULT_ZONE_URLS];
    zonesURL = zonesurls[0];
    coverURL = DEFAULT_COVER_URL;
    htmlURL = DEFAULT_HTML_URL;
    localStorage.removeItem('daknuxSourceSettings');
    document.getElementById('settingZonesURL').value = zonesurls.join('\n');
    document.getElementById('settingCoversURL').value = coverURL;
    document.getElementById('settingHTMLURL').value = htmlURL;
    alert("Reset to default sources!");
    listZones();
}
...
function openSource(which) {
    let url = "";
    if (which === 'zones') {
        const t = document.getElementById('settingZonesURL');
        url = t ? t.value.split(/\n/).map(u => u.trim()).filter(Boolean)[0] : zonesURL;
        url = url || zonesURL;
    } else if (which === 'covers') {
        const t = document.getElementById('settingCoversURL');
        url = (t && t.value.trim()) || coverURL;
    } else if (which === 'html') {
        const t = document.getElementById('settingHTMLURL');
        url = (t && t.value.trim()) || htmlURL;
    }
...
async function listZones() {
    try {
      ...
      let shajson;
      let sha;
      const repoInfo = parseGhUrl(zonesURL);
      if (repoInfo) {
        const repoPath = `${repoInfo.owner}/${repoInfo.repo}`;
        try {
          sharesponse = await fetch("https://api.github.com/repos/" + repoPath + "/commits?t="+Date.now());
        } catch (error) {}
        if (sharesponse && sharesponse.status === 200) {
          try {
            ...
            sha = shajson[0]['sha'];
            if (sha) {
              zonesURL = zonesURL.replace(/@[^\/]*/, `@${sha}`);
            }
          } catch (error) {
            try {
              const fallbackShaUrl = repoPath === "daknux/assets"
                  ? "https://raw.githubusercontent.com/daknux/xml/refs/heads/main/sha.txt?t="+Date.now()
                  : `https://raw.githubusercontent.com/${repoPath}/refs/heads/main/sha.txt?t=${Date.now()}`;
              const secondarysharesponse = await fetch(fallbackShaUrl);
        ...
        zones = Array.isArray(json) ? json : (Array.isArray(json.zones) ? json.zones : []);
        if (zones.length) zones[0].featured = true; // always gonna be the discord
                ...
                if (embed) {
                    if (zone.url.startsWith("http")) {
                        ...
                        fetch(url+"?t="+Date.now()).then(response => response.text()).then(html => {
                            ...
                            popup.innerHTML = `Play more games at <a href="https://daknux.github.io" target="_blank" style="color:#004085; font-weight:bold;">https://daknux.github.io</a>!`;
...
async function fetchPopularity(duration) {
    try {
        ...
        const htmlInfo = parseGhUrl(htmlURL);
        const statsPkg = htmlInfo ? `gh/${htmlInfo.owner}/${htmlInfo.repo}@${htmlInfo.version}` : "gh/daknux/html@main";
        const response = await fetch(
            "https://data.jsdelivr.com/v1/stats/packages/" + statsPkg + "/files?period=" + duration
        );
...
settings.addEventListener('click', () => {
    ...
    <br><br>
    <button class="settings-button" onclick="randomZone()">Random Game</button>
    <br><br><hr><br>
    <h3 style="margin:0 0 6px;">Content Sources</h3>
    <p style="margin:0 0 10px; font-size:0.85em;">Point these at your own repo by editing the GitHub username / repo in the jsDelivr URL (and the JSON file for zones).</p>
    <label for="settingZonesURL" style="font-weight:bold;">Zones URL (one per line):</label><br>
    <textarea id="settingZonesURL" rows="3" style="width:100%; box-sizing:border-box; resize:vertical; background:#1a1a1a; color:#eee; border:1px solid #555; border-radius:4px; padding:6px; font-family:monospace;">${esc(zonesurls.join('\n'))}</textarea>
    <button class="settings-button" onclick="openSource('zones')" style="margin-top:4px;">Go to zones source</button>
    <br><br>
    <label for="settingCoversURL" style="font-weight:bold;">Covers URL:</label><br>
    <input type="text" id="settingCoversURL" value="${esc(coverURL)}" style="width:100%; box-sizing:border-box; background:#1a1a1a; color:#eee; border:1px solid #555; border-radius:4px; padding:6px; font-family:monospace;"><br>
    <button class="settings-button" onclick="openSource('covers')" style="margin-top:4px;">Go to covers source</button>
    <br><br>
    <label for="settingHTMLURL" style="font-weight:bold;">HTML URL:</label><br>
    <input type="text" id="settingHTMLURL" value="${esc(htmlURL)}" style="width:100%; box-sizing:border-box; background:#1a1a1a; color:#eee; border:1px solid #555; border-radius:4px; padding:6px; font-family:monospace;"><br>
    <button class="settings-button" onclick="openSource('html')" style="margin-top:4px;">Go to html source</button>
    <br><br>
    <button class="settings-button" onclick="saveSourceSettings()">Save Sources</button>
    <button class="settings-button" onclick="resetSourceSettings()">Reset to Defaults</button>
    `;
    popupBody.contentEditable = false;
    document.getElementById('popupOverlay').style.display = "flex";
});
...
function loadDisclaimer() {
    document.getElementById('popupTitle').textContent = "CREDITS AND STUFF";
    const popupBody = document.getElementById('popupBody');
    popupBody.innerHTML = `
        <div class="CREDITS AND STUFF">
            <p>
                Credits to <strong>gn-math</strong> for many of the code from the site
            </p>
            <ol>
                <p>
                    Credits to <strong>gnmathcopy.github.io</strong> for the random game button stuff
                </p>
                <p>
                    Tools used to make daknux include UGS, github, many gh cdns, duck.ai and some other ais, ibis paint x for some thumbnails, and da$prunk leftovers
                </p>
            </ol>
            <p>
                <strong>DISCLAIMER</strong> DAKNUX is not an official product of Microsoft, So far so good, Rockstar games, Sega, Nintendo, Toby Fox, GN math, and other companies/UBGs <strong>WE ALSO DO NOT SELL GAMES</strong>
            </p>
            <p>
            The daknux logo DOES NOT REPRESENT ANY SHIPS OR FANDOMS, the logo is meant to remind folks about the tragedies of gn-math and many ubg sites on google sites after their strict base tag removal from their sandbox iframes
            </p>
        </div>
    `;
    popupBody.contentEditable = false;
...
async function getAllStats() {
  ...
  const BASE_URL =
    ...
    (htmlInfo ? `gh/${htmlInfo.owner}/${htmlInfo.repo}@${htmlInfo.version}` : "gh/daknux/html@main") +
    "/files";
  const PERIOD = "year";
  const PAGE_BATCH = 5;
...
function showZoneInfo() {
    ...
    document.getElementById('popupOverlay').style.display = "flex";
    const htmlInfo = parseGhUrl(htmlURL);
    const htmlRepo = htmlInfo ? `${htmlInfo.owner}/${htmlInfo.repo}` : "daknux/html";
    fetch(`https://api.github.com/repos/${htmlRepo}/commits?path=${id}.html`).then(res => res.json()).then(async json => {
  ...
  hour12: true
        ...
        <b>DAKNUX Adder</b>: ${json.at(-1).commit.author.name}<br>
