...
const coverURL = "https://cdn.jsdelivr.net/gh/daknux/covers@main";
const htmlURL = "https://cdn.jsdelivr.net/gh/daknux/html@main";
let zones = [];
let popularityData = {};
...
async function listZones() {
    try {
      ...
      let sha;
        try {
          sharesponse = await fetch("https://api.github.com/repos/daknux/assets/commits?t="+Date.now());
        } catch (error) {}
        if (sharesponse && sharesponse.status === 200) {
          try {
            ...
            sha = shajson[0]['sha'];
            if (sha) {
                zonesURL = `https://cdn.jsdelivr.net/gh/daknux/assets@${sha}/zones.json`;
            }
          } catch (error) {
            try {
                let secondarysharesponse = await fetch("https://raw.githubusercontent.com/daknux/xml/refs/heads/main/sha.txt?t="+Date.now());
...
async function fetchPopularity(duration) {
    try {
        ...
        const response = await fetch(
            "https://data.jsdelivr.com/v1/stats/packages/gh/daknux/html@main/files?period=" + duration
        );
...
async function getAllStats() {
  ...
  const BASE_URL =
    "https://data.jsdelivr.com/v1/stats/packages/gh/daknux/html@main/files";
  const PERIOD = "year";
  const PAGE_BATCH = 5;
...
function showZoneInfo() {
    ...
    popupBody.contentEditable = false;
    document.getElementById('popupOverlay').style.display = "flex";
    fetch(`https://api.github.com/repos/daknux/html/commits?path=${id}.html`).then(res => res.json()).then(async json => {
